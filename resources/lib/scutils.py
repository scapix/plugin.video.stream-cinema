...
class KODISCLib(xbmcprovider.XBMCMultiResolverContentProvider):
    ...
    def add_multi_item(self, params, addToSubscription=False):
        ...
        while data is not None:
            ...
            for i in data['list']:
                ...
                param['progressPct'] = perc
                if page > 1:
                    param['progressPage'] = page 
                if params['id'] == 'movies':
    ...
    def add_item_trakt(self, params, addToSubscription=False):
        ...
        if trakt.getTraktCredentialsInfo() == True:
            ...
            if 'menu' in data:
                ...
                try:
                    dialog = params['progressDialog']
                    util.debug('[SCAPE] add_item_trakt using already created progressDialog %s'%type(dialog))
                except:
                    ...
                    util.debug('[SCAPE] add_item_trakt created progressDialog')

                total = float(len(data['menu']))
                num = 0
                for i in data['menu']:
                    num += 1
                    perc = 100.0 * num / total
    ...
    def add_item_lastrun(self, params, ids):
        ...
        if data:
            for i in data:
                (e, n) = self.add_item(params, data=i)
                error |= e
                new_items |= n
        else:
            error = True
    ...
    def add_item(self, params, addToSubscription=False, data=None):
        ...
        if 'ep' not in data:
            ...
            for num, itm in enumerate(data['ep']):
                item_path = os.path.join(
                    item_dir, self.normalize_filename(data['title']),
                    'Season ' + itm['season'],
            ...
            if not self.scanRunning() and not self.isPlaying():
                ...
                if total > 0:
                    ...
                    if sdata is not None and 'data' in sdata and len(
                        ...
                        if len(ids) > 0:
                            (e, n) = self.add_item_lastrun({'progressDialog': dialog, 'progressPct': 100},ids)
                            error |= e
                            new_items |= n
                ...
                if sctop.getSettingAsBool('download-movies'):
                    ...
                    if self.canCheck(data['last_run']) or force is True:
                        ...
                        subs['movie'] = data
                        self.setSubs(subs)
                        self.add_multi_item({'progressDialog': dialog, 'id': 'movies'}, False)
                    else:
                ...
                if 'traktwatchlist' in subs and 'lists' in subs['traktwatchlist']:
                    ...
                    if len(data['lists']) > 0 and (self.canCheck(data['last_run']) or force is True):
                        ...
                        util.info(
                            "[SC] download trakt listy netreba stahovat")
